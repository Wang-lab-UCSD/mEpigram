"""usage: julia quickPssmScanBestMatchLiteTypeEF.jl memefile file.faa tagname resuldir bgComposition"""
#function score(kmer,PWM,background) #not used
...
function convertPWMtoMatrixEF(PWM)
...
function main()
    """This function takes in a memefile, a set of sequences, 
    and output the best score for each sequence by each motif in each separate file"""
    motiffile = ARGS[1]
    sequencefile = ARGS[2]
    tag = ARGS[3]
    resultdir = ARGS[4] 
    ...
    transdict=Dict('A'=>'T','C'=>'G','G'=>'C','T'=>'A','E'=>'F','F'=>'E', 'N'=>'N')
    transdict2=Dict('A'=>"1",'C'=>"2",'G'=>"3",'T'=>"4",'E'=>"5", 'F'=>"6",'N'=>"7")

    motifnames=keys(motifs)
    for m in motifnames
        pwm=motifs[m]
        pwmmatrix = convertPWMtoMatrixEF(pwm)
        println("Scanning "*m)
