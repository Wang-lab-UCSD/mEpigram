...
def estimateBackground(seqs,istypeEF):
	"""
	Estimate the background base composition from the sequences
	"""

	if istypeEF:
		countDict = {"A":0,"C":0,"G":0,"T":0,"E":0,"F":0}
		for s in seqs:
			s = s.upper()
			for a in countDict:
				countDict[a] += s.count(a)
	else:
		countDict = {"A":0,"C":0,"G":0,"T":0,"E":0}
		for s in seqs:
			s = s.upper()
			for a in countDict:
				countDict[a] += s.count(a)
...
def main():
	...
	if (backgroundfile == None):
		print "No background base composition provided, estimate background from inputs instead..."
		seqs = readSeqs(faafile)
		...
		if not (typeEF):
			...
			background = estimateBackground(seqs,False)
		else:
			...
			background = estimateBackground(seqs,True)
		#print background
